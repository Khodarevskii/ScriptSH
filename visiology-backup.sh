...
#
# Postgres, Smart Forms, MinIO, docker secrets и пользовательские настройки
# снимаются теми же командами, что и штатный backup.sh. ClickHouse
...
# custom-configs.
#
# Keycloak в бэкап НЕ входит: realm, пользователи и настройки внешнего SSO
# этим скриптом не сохраняются и не восстанавливаются. Восстанавливать архив
# следует с --with-keycloak false (значение по умолчанию в штатном restore.sh).
#
...
while [ "$1" != "" ]; do
    case "$1" in
        "-?" | "-h" | "--help")
            ...
            echo "  --ch-only     только ClickHouse: без postgres, minio и"
            ...
            echo "Keycloak не входит в бэкап."
            echo
