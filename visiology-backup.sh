...
# Снапшот и точка монтирования живут до конца прогона: вся уборка выполняется
# в cleanup, уже ПОСЛЕ сборки архива. Причина - umount на снапшоте, который
# держит антивирус, уходит в непрерываемый сон, и timeout такой процесс убить
# не может. В конце это никому не мешает, в середине вставало на 17 минут.
#
...
dump_clickhouse() {
    ...
    log "  снапшот и ${SNAP_MNT} остаются до конца прогона, уборка в самом конце"
